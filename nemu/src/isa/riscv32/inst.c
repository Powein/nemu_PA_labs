...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // my S series
  INSTPAT("?????? ?????? ????? 000 ????? 01000 11", sb     , S, vaddr_write(src1 + (sword_t)imm, (word_t)src2, 1));
  INSTPAT("?????? ?????? ????? 001 ????? 01000 11", sh     , S, vaddr_write(src1 + (sword_t)imm, (word_t)src2, 2));
  INSTPAT("?????? ?????? ????? 010 ????? 01000 11", sw     , S, vaddr_write(src1 + (sword_t)imm, (word_t)src2, 4));
  // my B series
  ...
  // my J series
  INSTPAT("?????? ?????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->snpc + (sword_t)imm; Log("imm is %x", imm););
